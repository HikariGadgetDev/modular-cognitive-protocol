...
MCP が対象とするのは、**失敗や仕様変更が日常的に発生する外部依存領域**です。  
特定用途の自動化ではなく、**壊れたあとも運用を継続できる構造**に焦点を当てます。

- ブラウザ操作を伴う **E2E テスト・UI 回帰テストの自動化**
- 仕様変更や不安定性を前提とした **外部 API 連携の長期運用**
  （監視・リトライ・フォールバック・スキーマ差分吸収）
- 法令・利用規約を遵守した範囲での **公開情報の取得・整理（OSINT）**
- Web ページやフィードからの **データ収集・集約基盤**
- 権利・契約条件を満たす範囲での **メディア解析・メタデータ生成**
- 不安定な外部環境に依存する **AI エージェント／自動処理システムの基盤設計**

MCP は「何を自動化するか」ではなく、  
**「壊れたあとにどう維持・修復できるか」**を設計対象とします。
